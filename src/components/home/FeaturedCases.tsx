    ...
    <section className="py-32 md:py-40 bg-white">
      <div className="max-w-[1440px] mx-auto px-8 lg:px-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          {cases.map((c) => (
            <Link key={c.slug} href={`/cases/${c.slug}`} className="group block">
              <div className="relative aspect-[4/3] overflow-hidden mb-8 float-shadow img-fade-bottom">
                <img
                  src={c.thumbnail}
                  alt={c.title}
                ...
                <div className="absolute top-4 left-4">
                  <span className="px-3 py-1.5 bg-white/90 backdrop-blur-sm text-black/45 text-xs font-medium tracking-wider uppercase">
                    {c.industry}
                  </span>
                </div>
              </div>
              <h3 className="text-xl font-bold text-[#1D1D1F] mb-3 group-hover:text-accent transition-colors tracking-tight">
                {c.title}
              </h3>
        ...
        <div className="mt-20">
          <Link
            href="/cases"
            className="inline-flex items-center gap-3 text-black/35 font-medium text-sm tracking-wider hover:text-black transition-colors group"
          >
            查看全部案例

    ...
    <>
      <section className="pt-40 pb-24 bg-[#F5F5F7]">
        <div className="max-w-[1440px] mx-auto px-8 lg:px-16">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-[#1D1D1F] tracking-tight leading-none mb-6">全链路服务</h1>
          <p className="text-lg text-text-tertiary font-light max-w-xl">覆盖实验室从设计到运维的每一个环节</p>
        </div>
      </section>
            ...
            {services.map((service, i) => (
              <div key={service.slug} className={`flex flex-col ${i % 2 === 0 ? "lg:flex-row" : "lg:flex-row-reverse"} gap-16 lg:gap-32 items-center`}>
                <div className="lg:w-1/2">
                  <div className="aspect-[4/3] bg-[#F5F5F7] overflow-hidden float-shadow img-fade-bottom">
                    <img src={service.image} alt={service.title} className="w-full h-full object-cover" />
                  </div>
                </div>
                <div className="lg:w-1/2">
                  <p className="text-accent text-xs font-medium tracking-[0.25em] uppercase mb-6">{service.icon} {service.title}</p>
                  <h2 className="text-2xl md:text-4xl font-bold text-[#1D1D1F] tracking-tight mb-8">{service.title}</h2>
                  ...
                  <Link href={`/services/${service.slug}`} className="inline-flex items-center gap-3 text-[#1D1D1F] font-medium text-sm tracking-wider hover:text-accent transition-colors group">
